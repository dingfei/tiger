...
public class TranslateVisitor implements ast.Visitor
{
	...
	public void visit(ast.exp.Call e)
	{
		...
		this.exp = new codegen.C.exp.Call(newid, exp, e.Id.id, args);
		return;
	}
	...
	{
		this.exp = new codegen.C.exp.NewObject(e.Id.id);
		return;
	}
	...
	{
		s.exp.accept(this);
		this.stm = new codegen.C.stm.Assign(s.Id.id, this.exp);
		return;
	}
	...
	{
		d.type.accept(this);
		this.dec = new codegen.C.dec.Dec(this.type, d.Id.id);
		return;
	}
	...
	{
		ClassBinding cb = this.table.get(c.Id.id);
		this.classes.add(new codegen.C.classs.Class(c.Id.id, cb.fields));
		this.vtables.add(new codegen.C.vtable.Vtable(c.Id.id, cb.methods));
		this.classId = c.Id.id;
	...
	{
		ClassBinding cb = this.table.get(c.Id.id);
		codegen.C.classs.T newc = new codegen.C.classs.Class(c.Id.id, cb.fields);
	...
	{
		this.table.init(((ast.mainClass.MainClass) m).Id.id, null);
	...
	public void scanClasses(java.util.LinkedList<ast.classs.T> cs)
	{
		...
		for (ast.classs.T c : cs)
		{
			...
			this.table.init(cc.Id.id, cc.extendss);
		}
		...
		for (ast.classs.T c : cs)
		{
			...
			this.table.initDecs(cc.Id.id, newDecs);
			...
			for (ast.method.T mthd : methods)
			{
				...
				this.table.initMethod(cc.Id.id, newRet, newArgs, m.id);
			}
		}
