...
public class Fac
{
	...
	// * System.out.println(new Fac().ComputeFac(10)); } } class Fac { public
	// int
	// * ComputeFac(int num) { int num_aux; if (num < 1) num_aux = 1; else
	// num_aux =
	...
	static ast.mainClass.MainClass factorial = new ast.mainClass.MainClass(
			new ast.exp.Id("Factorial"), new ast.exp.Id("a"),
			new ast.stm.Print(new ast.exp.Call(new ast.exp.NewObject(
					new ast.exp.Id("Fac")), new ast.exp.Id("ComputeFac"),
	...
	static ast.classs.Class fac = new ast.classs.Class(
			...
			new util.Flist<ast.method.T>().addAll(new ast.method.Method(
					...
					new util.Flist<ast.dec.T>().addAll(new ast.dec.Dec(
							new ast.type.Int(), new ast.exp.Id("num"))),
					new util.Flist<ast.dec.T>().addAll(new ast.dec.Dec(
							new ast.type.Int(), new ast.exp.Id("num_aux"))),
